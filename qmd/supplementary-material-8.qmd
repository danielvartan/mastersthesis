...
This document presents the analysis for testing the thesis hypothesis (**Test B**) using the methodology outlined in Supplementary Material [-@sec-sm-methods].
...
::: {.callout-tip}
Assumption 1
: \hspace{20cm} **Predictor is known**. Either the vectors $z_{1}, \dots , z_{n}$ are known ahead of time, or they are the observed values of random vectors $Z_{1}, \dots , Z_{n}$ on whose values we condition before computing the joint distribution of ($Y_{1}, \dots , Y_{n}$) [@degroot2012a, p. 736].
:::
...
::: {.callout-tip}
The development and analysis of the restricted model are detailed in Supplementary Material [-@sec-sm-restricted-model].
:::
...
::: {.callout-tip}
Assumption 2
: \hspace{20cm} **Normality**. For $i = 1, \dots, n$, the conditional distribution of $Y_{i}$ given the vectors $z_{1}, \dots , z_{n}$ is a normal distribution [@degroot2012a, p. 737].
...
::: {#tbl-sm-8-residual-diag-normality-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-8-residual-diag-normality-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-8-residual-diag-normality-3}
```{r}
#| code-fold: true
...
::: {#tbl-sm-8-residual-diag-normality-2}
```{r}
#| code-fold: true
...
::: {.callout-tip}
Assumption 3
: \hspace{20cm} **Linear mean**. There is a vector of parameters  $\beta = (\beta_{0}, \dots, \beta_{p - 1})$ such that the conditional mean of $Y_{i}$ given the values $z_{1}, \dots , z_{n}$ has the form
...
::: {#fig-sm-8-residual-diag-fitted-values-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-8-residual-diag-linearity-1}
```{r}
#| code-fold: true
...
::: {.callout-tip}
Assumption 4
: \hspace{20cm} **Common variance** (homoscedasticity). There is as parameter $\sigma^{2}$ such the conditional variance of $Y_{i}$ given the values $z_{1}, \dots , z_{n}$ is $\sigma^{2}$ for $i = 1, \dots, n$.
...
::: {#fig-sm-8-diag-homoscedasticity-1}
```{r}
#| code-fold: true
...
{{< include ../qmd/_tabset-panel-sm-8-var-homoscedasticity.qmd >}}
...
::: {.callout-tip}
Assumption 5
: \hspace{20cm} **Independence**. The random variables $Y_{1}, \dots , Y_{n}$ are independent given the observed $z_{1}, \dots , z_{n}$ [@degroot2012a, p. 737].
...
@tbl-sm-8-residual-diag-independence-1 shows the relative importance of independent variables in determining the response variable. It highlights how much each variable uniquely contributes to the R-squared value, beyond what is explained by the other predictors.

::: {#tbl-sm-8-residual-diag-independence-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-8-residual-diag-collinearity-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-8-residual-diag-collinearity-2}
```{r}
#| code-fold: true
...
::: {#tbl-sm-8-residual-diag-collinearity-3}
```{r}
#| code-fold: true
...
{{< include ../qmd/_tabset-panel-sm-8-coef-dfbetas.qmd >}}
...
Based on the hypothesis test results and the methodology outlined in Supplementary Material [-@sec-sm-methods], **we fail to reject the null hypothesis**. Latitude does not meaningfully contribute to explaining the variance in chronotype.
