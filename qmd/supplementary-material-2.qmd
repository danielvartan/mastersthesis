...
Text/character variables: `track`, `names`, `email`, `country`, `state`, `municipality`, `postalcode`, `sleep_drugs_which`, `sleep_disorder_which`, `medication_which`.
...
In the end, the final values of the `msf_sc` variable were all after the midnight, so the treatment was not necessary.
...
- **Null Hypothesis** ($\text{H}_{0}$): Including *latitude* as a predictor does not result in a meaningful improvement in the model’s fit, as evidenced by a change in the adjusted $\text{R}^{2}$ that is less than the Minimum Effect Size (MES).

- **Alternative Hypothesis** ($\text{H}_{a}$): Including *latitude* as a predictor results in a meaningful improvement in the model’s fit, as evidenced by a change in the adjusted $\text{R}^{2}$ that meets or exceeds the Minimum Effect Size (MES).
