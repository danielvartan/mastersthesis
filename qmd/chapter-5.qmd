...
::: {.callout-note}
The following study was designed for publication in the journal [*Scientific Reports*](https://www.nature.com/srep/) ([IF 2023: 3.8/JCR](https://jcr.clarivate.com/jcr) | [A1/2017–2020](https://sucupira-legado.capes.gov.br/sucupira/)) and structured in accordance with the journal's [submission guidelines](https://www.nature.com/srep/author-instructions/submission-guidelines).
:::
:::
...
This article brings a novel attempt to test the latitude hypothesis, using, this time, a biological approach provided by the Munich ChronoType Questionnaire (MCTQ) [@roenneberg2003]. Furthermore, the test was carried out on the biggest chronotype sample ever collected in a same country. A sample made of `{r} weighted_data |> nrow() |> format(big.mark = ",") |> format_to_md_latex()` subjects, all living in the same timezone in Brazil, with only one week of difference between questionnaire responses [@fig-chapter-5-sample-geographical-distribution]. The unfiltered valid sample comprises `{r} geocoded_data |> nrow() |> format(big.mark = ",") |> format_to_md_latex()` participants from all Brazilian states.
...
Geographical distribution of the sample used in the analysis: (`{r} weighted_data |> nrow() |> format(big.mark = ",") %>% paste0("n = ", .) |> format_to_md_latex()`). Each point represents a municipality, with its size proportional to the number of participants and its color indicating participant density. The sample includes Brazilian individuals aged 18 or older, residing in the UTC-3 timezone, who completed the survey between October 15 and 21, 2017. Size and color scale are in $\log_{10}$.
:::
...
It's also important to point out that this seemly weak relationship shown in @fig-chapter-5-chronotype-latitude-series is of very similar magnitude from the one @leocadio-miguel2017 found (see @leocadio-miguel2017, Figure 2), with the difference that it isn't masked by a distorted y-axis — see the supplemental materials to view a resescaled version of the mentioned figure.
...
::: {#fig-chapter-5-analysis-sample-geo-distribution-by-chronotype}
```{r}
#| message: false
#| warning: false
...
Geographical distribution of the sample used in the analysis: (`{r} weighted_data |> nrow() |> format(big.mark = ",") %>% paste0("n = ", .) |> format_to_md_latex()`) grouped by chronotypes.
:::
