...
The Munich Chronotype Questionnaire (MCTQ) uses the midpoint between sleep onset (SO) and sleep end (SE) on work-free days (MSF~sc~), with a sleep correction (sc) applied if sleep debt is detected, as a proxy for chronotype [@roenneberg2003]. For example, if an individual sleeps from $\text{00:00}$ to $\text{08:00}$, the midpoint would be  $\text{04:00}$. This measure is based on the current understanding of sleep regulation, which comprises a homeostatic/sleep-dependent process ($\text{S}$ process) and a circadian process ($\text{C}$ process) [@borbely1982; @borbely2016]. The midpoint of sleep on free days offers a way to observe unrestrained sleep behavior, thereby minimizing the influence of the $\text{S}$ process and providing a better approximation of the circadian phenotype (i.e., the $\text{C}$ process).
...
[Source: Created by the author, based on a data visualization from `<br>`{=html} `\\`{=latex} @roenneberg2019b[Figure 1, right part].]{.legend}
...
This represents the midsleep point for Brazilian subjects living in the UTC-3 timezone, with an intermediate or average chronotype. Considering the $7–9$ hours of sleep recommended for healthy adults by the American Academy of Sleep Medicine (AASM) [@watson2015], one might infer that this average individual, in the absence of social constraints, would typically wake up at approximately `{r} res_vars$pr_analysis_sample_msf_sc_mean |> hms::as_hms() |> lubritime::sum_time(hms::parse_hm("04:00")) |> as.numeric() |> hms::as_hms() |> format_to_md_latex(before = "\\text{", after = "}")`.
...
Chronotypes were assessed using a sleep log based on the core version of the Munich ChronoType Questionnaire (MCTQ) [@roenneberg2003], a well-validated and widely used self-report tool for measuring sleep-wake behavior and determining chronotype [@roenneberg2019]. The MCTQ derives chronotype from the sleep-corrected midpoint of sleep on free days (MSF~sc~), which compensates for sleep debt incurred during the workweek [@roenneberg2012]. @fig-chapter-5-mctq-variables illustrates the variables collected by the MCTQ.
...
The sample latitudinal range is `{r} res_vars$ea_analysis_sample_lat_lon$latitude[3] |> format_to_md_latex(max_digits = 5)` decimal degrees (`{r} res_vars$ea_analysis_sample_lat_lon$latitude[1] |> format_to_md_latex(before = "\\text{Min.} = ", max_digits = 5)`, `{r} res_vars$ea_analysis_sample_lat_lon$latitude[2] |> format_to_md_latex(before = "\\text{Max.} = ", max_digits = 5)`) with a longitudinal span of `{r} res_vars$ea_analysis_sample_lat_lon$longitude[3] |> format_to_md_latex(max_digits = 5)` (`{r} res_vars$ea_analysis_sample_lat_lon$longitude[1] |> format_to_md_latex(before = "\\text{Min.} = ", max_digits = 5)`, `{r} res_vars$ea_analysis_sample_lat_lon$longitude[2] |> format_to_md_latex(before = "\\text{Max.} = ", max_digits = 5)`). For comparison, Brazil has a latitudinal range of `{r} res_vars$ea_brazil_lat_lon$latitude[3] |> format_to_md_latex(max_digits = 5)` decimal degrees (`{r} res_vars$ea_brazil_lat_lon$latitude[1] |> format_to_md_latex(before = "\\text{Min.} = ", max_digits = 5)`; `{r} res_vars$ea_brazil_lat_lon$latitude[2] |> format_to_md_latex(before = "\\text{Max.} = ", max_digits = 5)`) and a longitudinal span of `{r} res_vars$ea_brazil_lat_lon$longitude[3] |> format_to_md_latex(max_digits = 5)` (`{r} res_vars$ea_brazil_lat_lon$longitude[1] |> format_to_md_latex(before = "\\text{Min.} = ", max_digits = 5)`; `{r} res_vars$ea_brazil_lat_lon$longitude[2] |> format_to_md_latex(before = "\\text{Max.} = ", max_digits = 5)`), according to data from IBGE collected via the [geobr](https://ipeagit.github.io/geobr/index.html>) R package [@pereira].

Additional details about the sample are available in the supplementary materials.

### Power Analysis

To assess the adequacy of the sample size for detecting effects reaching the Minimum Effect Size (MES) threshold ($f^2 = 0.02$), we conducted an *a posteriori* power analysis using the [pwrss](https://cran.r-project.org/web/packages/pwrss/vignettes/examples.html) R package [@bulus]. This analysis revealed a minimum sample size of `{r} res_vars$hta_pwr_analysis$n |> format_to_md_latex()` observations per variable to achieve a power ($1 - \beta$) of $0.99$ with a significance level ($\alpha$) of $0.01$. Our large sample size (`{r} res_vars$ea_analysis_sample_n |> format_to_md_latex(before = "n = ")`) comfortably surpasses this threshold, ensuring adequate power.

### Data Wrangling

Data wrangling and analysis followed the data science framework proposed by Hadley Wickham and Garrett Grolemund [@wickham2023d]. All processes were conducted using the R programming language [@rcoreteam], the RStudio IDE [@positteam], and several R packages. The [tidyverse](https://www.tidyverse.org/) and [rOpenSci](https://ropensci.org/) peer-reviewed package ecosystem and other R packages adherents of the tidy tools manifesto [@wickham2023] were prioritized.

The MCTQ data was analyzed using the [mctq](https://docs.ropensci.org/mctq/) R package [@vartanianf], which is part of the rOpenSci peer-reviewed ecosystem. The data pipeline was built using the rOpenSci peer-reviewed [targets](https://books.ropensci.org/targets/) R package [@landau2021], which provides a reproducible and efficient workflow for data analysis.

All processes were designed to ensure result reproducibility and adherence to the FAIR principles (Findability, Accessibility, Interoperability, and Reusability) [@wilkinson2016]. All analyses are fully reproducible and were conducted using [Quarto](https://quarto.org/) computational notebooks. The [renv](https://rstudio.github.io/renv/) R package [@ushey] was employed to ensure that the R analysis environment can be reliably restored.

### Hypothesis Test

To test the study hypothesis, nested multiple linear regression models were compared: a restricted model (excluding latitude) and a full model (including latitude). To ensure practical significance, a Minimum Effect Size (MES) criterion was applied, in line with the original Neyman-Pearson framework for hypothesis testing [@neyman1928; @neyman1928a; @perezgonzalez2015]. The MES was set at a Cohen's threshold for small effects ($f^2 = 0.02$, equivalent to $\text{R}^2 = 0.01960784$) ("just barely escaping triviality" [@cohen1988, p. 413]). Consequently, latitude was considered significant only if its inclusion explained at least $1.960784\%$ of the variance in the dependent variable.
