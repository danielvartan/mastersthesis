...
A chronotype is a temporal phenotype [@ehret1974a; @pittendrigh1993], typically used to refer to endogenous circadian rhythms---biological rhythms with periods close to 24 hours. Chronobiology, the science that studies biological rhythms, suggests that the evolution of these internal oscillators is closely linked to our environment, particularly to the day/night cycle. Is believed that this cycle, alongside human evolution, created environmental pressures that led to the development of temporal organization within organisms [@pittendrigh1981; @aschoff1989; @paranjpe2005]. Such organization allowed organisms to predict events and better manage their needs, such as storing food for winter [@aschoff1989a].
...
The sample latitudinal range is `{r} res_vars$ea_analysis_sample_lat_lon$latitude[3] |> groomr:::format_to_md_latex(after = "°", max_digits = 5)` (`{r} res_vars$ea_analysis_sample_lat_lon$latitude[1] |> groomr:::format_to_md_latex(before = "\\text{Min.} = ", after = "°", max_digits = 5)`, `{r} res_vars$ea_analysis_sample_lat_lon$latitude[2] |> groomr:::format_to_md_latex(before = "\\text{Max.} = ", after = "°", max_digits = 5)`) with a longitudinal span of `{r} res_vars$ea_analysis_sample_lat_lon$longitude[3] |> groomr:::format_to_md_latex(after = "°", max_digits = 5)` (`{r} res_vars$ea_analysis_sample_lat_lon$longitude[1] |> groomr:::format_to_md_latex(before = "\\text{Min.} = ", after = "°", max_digits = 5)`, `{r} res_vars$ea_analysis_sample_lat_lon$longitude[2] |> groomr:::format_to_md_latex(before = "\\text{Max.} = ", after = "°", max_digits = 5)`). For comparison, Brazil has a latitudinal range of `{r} res_vars$ea_brazil_lat_lon$latitude[3] |> groomr:::format_to_md_latex(after = "°", max_digits = 5)` (`{r} res_vars$ea_brazil_lat_lon$latitude[1] |> groomr:::format_to_md_latex(before = "\\text{Min.} = ", after = "°", max_digits = 5)`; `{r} res_vars$ea_brazil_lat_lon$latitude[2] |> groomr:::format_to_md_latex(before = "\\text{Max.} = ", after = "°", max_digits = 5)`) and a longitudinal span of `{r} res_vars$ea_brazil_lat_lon$longitude[3] |> groomr:::format_to_md_latex(after = "°", max_digits = 5)` (`{r} res_vars$ea_brazil_lat_lon$longitude[1] |> groomr:::format_to_md_latex(before = "\\text{Min.} = ", after = "°", max_digits = 5)`; `{r} res_vars$ea_brazil_lat_lon$longitude[2] |> groomr:::format_to_md_latex(before = "\\text{Max.} = ", after = "°", max_digits = 5)`), according to data from IBGE collected via the [geobr](https://ipeagit.github.io/geobr/index.html>) R package [@pereira].
