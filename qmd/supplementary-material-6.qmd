<!-- %:::% .common h1 begin %:::% -->
# Hypothesis Test B {#sec-sm-hypotehesis-test-b}
<!-- %:::% .common h1 end %:::% -->
...
As with all analyses in this thesis, the process is fully reproducible and was conducted using the [R programming language](https://www.r-project.org/) [@rcoreteam] along with the [Quarto](https://quarto.org/) publishing system [@allaire].
...
The results indicate that at least `{r} pwr_analysis$n |> format_to_md_latex()` observations per variable are required to achieve a power of $0.99$ ($1 - \beta$) (the probability of **not** committing a [type II error](https://en.wikipedia.org/wiki/Type_I_and_type_II_errors)) and a significance level ($\alpha$) of $0.01$ ($0.99$ probability of **not** committing a [type I error](https://en.wikipedia.org/wiki/Type_I_and_type_II_errors)). The dataset contains `{r} format_to_md_latex(nrow(data))` observations, which exceeds this requirement.
...
{{< include ../qmd/_panel-tabset-var-distribution-sm-6.qmd >}}

## Examining Correlations

::: {#fig-sm-6-correlation-matrix}
```{r}
data |> 
  plot_ggally(
    cols = c(
...
::: {#tbl-sm-6-restricted-model-fit-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-restricted-model-fit-2}
```{r}
#| code-fold: true
...
write_in_results_yml(
  list(
    htb_restricted_model_fit_coefs = fit_coefs,
    htb_restricted_model_fit_stats = fit_stats
  )
)
```
...
::: {#fig-sm-6-restricted-model-fit-comparison}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-residual-diag-normality-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-residual-diag-normality-2}
```{r}
#| code-fold: true
...
Boxplot of model residuals with outliers highlighted in red.
:::

::: {#tbl-sm-6-restricted-model-residual-diag-normality-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-restricted-model-residual-diag-normality-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-residual-diag-fitted-values-1}
```{r}
#| code-fold: true
...
Residual plot showing the relationship between fitted values and residuals. The dashed black line represent zero residuals, indicating an ideal model fit. The red line indicate the conditional mean of residuals.
:::

::: {#fig-sm-6-restricted-model-residual-diag-linearity-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-homoscedasticity-1}
```{r}
#| code-fold: true
...
data |>
  panel_tabset_var_homoscedasticity(
    data_name = "data",
    fit = fit,
    cols = c(
      "msf_sc",
    ...
    heading = "######",
    suffix = "sm-6-restricted-model"
  )
```

{{< include ../qmd/_panel-tabset-var-homoscedasticity-sm-6-restricted-model.qmd >}}
...
::: {#fig-sm-6-restricted-model-diag-independence-1}
```{r}
#| code-fold: true
...
@tbl-sm-6-restricted-model-residual-diag-independence-1 shows the relative importance of independent variables in determining the response variable. It highlights how much each variable uniquely contributes to the R-squared value, beyond what is explained by the other predictors.

::: {#tbl-sm-6-restricted-model-residual-diag-independence-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-restricted-model-residual-diag-colinearity-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-restricted-model-residual-diag-colinearity-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-3}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-4}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-5}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-6}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-7}
```{r}
#| code-fold: true
...
for (i in c(1:9)) {
  ...
  plot$data[[i]]$colour <- dplyr::case_when(
    plot$data[[i]]$colour == "blue" ~ 
      ifelse(i == 4, get_brand_color("grey"), get_brand_color("orange")),
...
::: {#fig-sm-6-restricted-model-diag-influence-8}
```{r}
#| code-fold: true
...
fit_engine |>
  panel_tabset_coef_dfbetas(
    heading = "######",
    suffix = "sm-6-restricted-model"
  )
```

{{< include ../qmd/_panel-tabset-coef-dfbetas-sm-6-restricted-model.qmd >}}
...
::: {#fig-sm-6-restricted-model-diag-influence-12}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-restricted-model-diag-influence-13}
```{r}
#| code-fold: true
...
write_in_results_yml(
  list(
    htb_full_model_fit_coefs = fit_coefs,
    htb_full_model_fit_stats = fit_stats
  )
)
```
...
::: {#fig-sm-6-full-model-fit-comparison}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-residual-diag-normality-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-residual-diag-normality-2}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-full-model-residual-diag-normality-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-full-model-residual-diag-normality-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-residual-diag-fitted-values-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-residual-diag-linearity-1}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-homoscedasticity-1}
```{r}
#| code-fold: true
...
data |>
  panel_tabset_var_homoscedasticity(
    ...
    heading = "######",
    suffix = "sm-6-full-model"
  )
```

{{< include ../qmd/_panel-tabset-var-homoscedasticity-sm-6-full-model.qmd >}}
...
::: {#fig-sm-6-full-model-diag-independence-1}
```{r}
#| code-fold: true
...
@tbl-sm-6-full-model-residual-diag-independence-1 shows the relative importance of independent variables in determining the response variable. It highlights how much each variable uniquely contributes to the R-squared value, beyond what is explained by the other predictors.

::: {#tbl-sm-6-full-model-residual-diag-independence-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-full-model-residual-diag-colinearity-1}
```{r}
#| code-fold: true
...
::: {#tbl-sm-6-full-model-residual-diag-colinearity-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-1}
```{r}
#| code-fold: true
...
dplyr::tibble(
  ...
  ggplot2::geom_linerange(color = "black") +
  ggplot2::geom_hline(yintercept = 2, color = get_brand_color("grey")) +
  ggplot2::geom_hline(yintercept = -2, color = get_brand_color("grey")) +
  ggplot2::geom_hline(yintercept = 3, color = get_brand_color("orange")) +
  ggplot2::geom_hline(yintercept = -3, color = get_brand_color("orange")) +
  ggplot2::scale_y_continuous(breaks = seq(-3, 3)) +
  ggplot2::labs(
    x = "Observation",
...
::: {#fig-sm-6-full-model-diag-influence-2}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-3}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-4}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-5}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-6}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-7}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-8}
```{r}
#| code-fold: true
...
{{< include ../qmd/_panel-tabset-coef-dfbetas-sm-6-full-model.qmd >}}
...
::: {#fig-sm-6-full-model-diag-influence-12}
```{r}
#| code-fold: true
...
::: {#fig-sm-6-full-model-diag-influence-13}
```{r}
#| code-fold: true
...
write_in_results_yml(
  list(
    htb_restricted_model_adj_r_squared = adj_r_squared_restricted,
    htb_full_model_adj_r_squared = adj_r_squared_full,
    htb_effect_size = effect_size
  )
)
```
...
::: {style="text-align: center; font-size: 1.1em; font-weight: bold; color: #EA77011; padding-top: 1em; padding-bottom: 1em;"}
